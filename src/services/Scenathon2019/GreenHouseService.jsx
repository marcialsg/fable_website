...
const responseApi = response => {
  ...
  if (response.length !== 0) {
    ...
    // iterate over FAO_GHGagric and if the value is 0, then replace it with null
    for (var i = 0; i < FAO_GHGagric.length; i++) {
      if (Number(FAO_GHGagric[i]) === 0) {
        FAO_GHGagric[i] = null;
      }
    }
    
    
    // iterate over FAO_GHG_LU and if the value is 0, then replace it with null
    for (var i = 0; i < FAO_GHG_LU.length; i++) {
      if (Number(FAO_GHG_LU[i]) === 0 && i !== 0) {
        FAO_GHG_LU[i] = null;
      }
    }
...
export default function getGreenHouseOne(url, props) {
  try {
