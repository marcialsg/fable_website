...
const drawMixedChart2 = (props) => {
	...
	const options = {
		...
		tooltips: {
			mode: 'label',
			callbacks: {
				label: function (tooltipItem, data) { return (tooltipItem.yLabel * 100).toFixed(1) + '%'; }
			},
		},
		elements: {
			line: {
	...
	return (
		<div class="prueba">
			...
			<div className="string-title" style={{ maxHeight: '500px', fontSize: '12px', fontWeight: 'bold' }}>{props.title || <></>}</div>
		</div>
	);
}
